 ...
 *   nochar    — 0 characters
 *   onechar   — 1 character  (Solo Pilot)
 *   twochars  — 2 characters (Alpha, Beta)
 */
...
async function main(): Promise<void> {
  console.log("[ucgo-login-server] Starting...");
  console.log(`[ucgo-login-server] XOR table: ${XOR_TABLE_PATH}`);
  ...
  const shutdown = (): void => {
    console.log("\n[ucgo-login-server] Shutting down...");
...
main().catch((err: unknown) => {
  ...
  console.error(`[ucgo-login-server] Fatal: ${msg}`);
  process.exit(1);
});
